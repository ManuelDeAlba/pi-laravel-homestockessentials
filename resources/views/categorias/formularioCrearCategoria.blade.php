<x-layout titulo="Crear categoría">
    <div class="container mx-auto p-4 grid gap-2">

        {{-- Cuando hay un error, los inputs se reinician, para eso se usa value="{{ old('name') }}" --}}        
        <h1 class="text-2xl text-center font-bold">Crear categoría</h1>

        <form class="flex flex-col gap-2" action="{{ route('categorias.store') }}" method="POST">
            @csrf
